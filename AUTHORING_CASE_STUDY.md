...
> ⚠️ **This case study was written in the sibling `lt_web` repo, before the
> Eldoria fork existed.** The commit hashes and the DEBUG/TRIAL/TOWN/RIVER
> levels below live in `lt_web`'s history, not Eldoria's — they are **not**
> playable here. The *techniques* (entity authoring, region/event wiring,
> the full-map-PNG tilemap pipeline, AI art generation) are the reason this
> doc is kept, and those transfer directly to Eldoria.
...
All five were playable via `?level=DEBUG`, `?level=TRIAL`, `?level=TOWN`,
`?level=RIVER` in `lt_web` at the time this was written (see the disclaimer
above).
