...
class AddSkillCheck(EventCommand):
    nid = 'add_skill_check'
    tag = Tags.LEVEL_VARS

    desc = \
        """
Adds the ability for the two indicated units to attempt a non-combat *Skill Check* against each other in the current chapter (e.g. persuading a bandit, talking down a guard). The first *Unit* will be able to initiate the check against the second *Unit*. The check is resolved on a d20 (see `roll_d20`) against *DC* (difficulty class), which defaults to 12 if omitted. When resolved, fires the `on_skill_check` trigger carrying the roll result -- see CONTENT_GUIDE.md for how to branch an event on it.
        """

    keywords = ["Unit1", "Unit2"]
    optional_keywords = ["DC"]
    keyword_types = ["Unit", "Unit", "PositiveInteger"]

class RemoveSkillCheck(EventCommand):
    nid = 'remove_skill_check'
    tag = Tags.LEVEL_VARS

    desc = \
        """
Removes the ability for the two indicated units to attempt a *Skill Check* against each other. You probably want to use this after the check resolves (in the `on_skill_check` event) so it can't be repeated -- mirrors **remove_talk**.
        """
